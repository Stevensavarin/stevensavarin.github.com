...
export async function loadRecipeDetail() {
    ...
    try {
        console.log(`recetaDetalle.mjs: Fetching recipes.json from: ${REPO_BASE_URL}public/data/recipes.json`);
        const res = await fetch(`${REPO_BASE_URL}public/data/recipes.json`); 
        if (!res.ok) {
          console.error(`recetaDetalle.mjs: Error fetching recipes.json. Status: ${res.status}`);
